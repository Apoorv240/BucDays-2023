...
public class Distance2 {
    ...
    public Distance2 rot(Angle rhs) {
        double sin = rhs.sin();
        double cos = rhs.cos();
        return new Distance2(this.y.mul(sin).add(this.x.mul(cos)), this.y.mul(cos).sub(this.x.mul(sin)));
    }

    public Distance magnitude() {
        return Distance.inMM(Math.hypot(this.x.valInMM(), this.y.valInMM()));
    }
